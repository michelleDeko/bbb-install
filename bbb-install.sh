...
HERE
    ...
    if [ -z "$PROVIDED_CERTIFICATE" ]; then
      if ! certbot --non-interactive --email "$EMAIL" --agree-tos --rsa-key-size 4096 -w /var/www/bigbluebutton-default/assets/ \
           -d "$HOST" --deploy-hook "systemctl reload nginx" --preferred-challenges http "${LETS_ENCRYPT_OPTIONS[@]}" certonly; then
        systemctl restart nginx
